<div x-data="{ scrollToBottomOfMessage: () => { $nextTick(() => { $refs.endOfMessage.scrollIntoView() }) } }" @scroll-to-bottom.window="scrollToBottomOfMessage();" x-init="scrollToBottomOfMessage()" class="p-3 gap-3 lg:col-span-1 bg-base-100">
        ...
        <div class="flex-grow max-h-[calc(100vh-12.5rem)] overflow-y-scroll">
            ...
            @else
                ...
                @foreach ($messages as $message)
                    <div class="chat {{ $message->from_user ? 'chat-start' : 'chat-end' }}">
                        <div class="chat-image avatar">
                            <div class="w-10 rounded-full overflow-hidden">
                                <img class="object-cover" src="{{ asset($message->sender->avatar) }}">
                            </div>
                        </div>
                        <div class="chat-header">
                            {{ $message->sender->display_name }}
                            <time class="text-xs opacity-50">{{ $message->created_at_label }}</time>
                        </div>

                        @if ($message->type === 'text' || $message->type === 'file')
                            <div class="chat-bubble break-words">{!! nl2br($message->body) !!}</div>
                        @else
                            <div class="chat-bubble">
                                <div class="pswp-gallery" id="my-gallery">
                                    <div class="grid grid-cols-1 gap-4 pswp-gallery">
                                        <a href="{{ Storage::disk('s3')->url($message->path) }}" target="_blank" class="w-full h-48 bg-gray-200 overflow-hidden">
                                            <img src="{{ Storage::disk('s3')->url($message->path) }}" alt="Image 1" class="object-cover w-full h-full">
                                        </a>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="flex justify-center flex-row">
                    <button wire:click="handleClientSelected('{{ $currentClientLineUserId }}')" class="btn rounded-full btn-ghost">
                        ...
                        <span class="text-base-content/70">最新メッセージ</span>
                    </button>
                </div>
            @endempty

            <div x-ref="endOfMessage"></div>
        </div>
    </div>
    @empty(!$messages)
        <div x-data="{ inputText: '', fileSelected: false, isLoading: false }" @file-uploaded.window="isLoading = false" class="flex w-full py-2 gap-2 relative">
            @if (!empty($allFiles))
                <a x-on:click="$dispatch('open-upload-files-modal');">
            ...
            @else
                <a onclick="inputMessageFiles.click()">
                    <svg class="w-8 my-2" data-slot="icon" fill="black" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                        <path clip-rule="evenodd" fill-rule="evenodd" d="M10 18a8 8 0 1 0 0-16 8 8 0 0 0 0 16Zm.75-11.25a.75.75 0 0 0-1.5 0v2.5h-2.5a.75.75 0 0 0 0 1.5h2.5v2.5a.75.75 0 0 0 1.5 0v-2.5h2.5a.75.75 0 0 0 0-1.5h-2.5v-2.5Z"></path>
                    </svg>
                </a>
            ...
            <div class="form-control w-full">
                <textarea rows="3" wire:model.lazy="text" x-model="inputText" type="text" placeholder="メッセージ" class="w-full resize-none input input-bordered pt-1 px-1.5"></textarea>
                @error('text')
            ...
            <button wire:click='postMsg()' type="button" :disabled="(inputText.trim() === '' && !fileSelected) || isLoading" class="btn btn-primary"><span x-show="!isLoading">送信</span><span x-show="isLoading" class="loading loading-spinner loading-md"></span></button>

            <div x-data="{ modalOpen: false }" x-on:open-upload-files-modal.window="modalOpen = true;" @keydown.escape.window="modalOpen = false" @organisations-updated.window="modalOpen = false" class="relative z-50 w-auto h-auto">
                <div x-show="modalOpen" class="fixed top-0 left-0 z-[99] flex items-center justify-center w-screen h-screen" x-cloak>
                    ...
                    <div x-show="modalOpen" x-trap.inert.noscroll="modalOpen" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="relative w-full py-6 bg-base-100 px-7 sm:max-w-4xl sm:rounded-lg">
                        ...
                        <div class="w-full">
                            <button tabindex="-1" @click="inputMessageFiles.click()" class="btn btn-neutral" wire:loading.attr="disabled">ファイルを追加する</button>
                            <input id="inputMessageFiles" wire:model="files" type="file" @change="fileSelected = $event.target.files.length > 0; isLoading = true;" multiple style="display:none;" tabindex="-1" />
                            @if ($allFiles)
